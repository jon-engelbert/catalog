...
class UserViewsTests(TestCase):
    ...
    def test_add_menuitem(self):
        ...
        with app.test_client() as c:
            with c.session_transaction() as sess:
                sess['user_id'] = 1
            response = c.post('/category/1/menu/new/', data=dict(name = 'yummy dog', description= 'you must try this', price= 1, course = 'dessert'), follow_redirects=True)
        print("response: {}".format(response.__dict__))
    ...
    def test_add_category(self):
        ...
        with app.test_client() as c:
            with c.session_transaction() as sess:
                sess['user_id'] = 1
            # print ("In test_add_category: {}".format(app.login_session))
            # app.login_session['user_id'] = 1
            print ("In test_add_category: {}".format(login_session))
            response = c.post('/category/new/', data={'name': 'soups r us'}, follow_redirects=True)
        print("response: {}".format(response.__dict__))
        print("response[status]: {}".format(response.status_code))
        assert self.db.session.query(Category).all() != None
